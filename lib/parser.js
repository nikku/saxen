...
export default function Parser(options) {
  ...
  /**
   * Parse in lax mode, skipping expensive well-formedness
   * checks to trade strictness for parse speed.
   *
   * This drops the per-element duplicate attribute check,
   * avoiding a `Set` allocation and lookups on the hot path.
   * Cheap recoverable warnings (malformed attributes, non-whitespace
   * outside the root node, ...) are still emitted.
   *
   * @type {boolean}
   */
  var lax = options && options['lax'];
